...
nginx_url="https://nginx.org/download/nginx-1.26.2.tar.gz"
vixiecron_url="https://github.com/vixie/cron/archive/refs/heads/master.tar.gz"
isc_dhcp_url="https://ftp.isc.org/isc/dhcp/4.4.3-P1/dhcp-4.4.3-P1.tar.gz"
...
openssh_url="https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-9.9p2.tar.gz"
...
bison flex libelf-dev gawk cpio \
...
# OpenSSH: servidor/cliente SSH estándar, en vez de una alternativa
# liviana. Sin PAM (para no meter todo ese stack por un solo uso);
# autentica directo contra /etc/shadow, igual que nuestro login propio.
answer="n"
if [ -f files/openssh/.built ]; then
printf "** Se detectó una compilación previa de openssh. ¿Reutilizarla? (y/n): "
read answer
fi
if [ "$answer" != "y" ]; then
cd files
fetch_source "$openssh_url" openssh-src.tar.gz
extract_source openssh-src.tar.gz openssh
cd openssh
FORCE_UNSAFE_CONFIGURE=1 CFLAGS="-O2" \
./configure --prefix=/usr --sysconfdir=/etc/ssh --without-pam --disable-strip \
|| { echo "** Falló el configure de openssh."; exit 1; }
make -j"$(nproc)" || { echo "** Falló la compilación de openssh."; exit 1; }
touch .built
cd ../..
fi
...
# nginx: sin módulo de rewrite (evita depender de PCRE) ni SSL por ahora;
# servidor web estándar en vez de uno hecho a medida.
answer="n"
if [ -f files/nginx/.built ]; then
printf "** Se detectó una compilación previa de nginx. ¿Reutilizarla? (y/n): "
read answer
fi
if [ "$answer" != "y" ]; then
cd files
fetch_source "$nginx_url" nginx-src.tar.gz
extract_source nginx-src.tar.gz nginx
cd nginx
...
|| { echo "** Falló el configure de nginx."; exit 1; }
make -j"$(nproc)" || { echo "** Falló la compilación de nginx."; exit 1; }
touch .built
cd ../..
fi

# vixie cron: implementación estándar de cron, en vez de un fork
# personal poco conocido (dcron).
answer="n"
if [ -f files/vixiecron/.built ]; then
printf "** Se detectó una compilación previa de vixie cron. ¿Reutilizarla? (y/n): "
read answer
fi
if [ "$answer" != "y" ]; then
cd files
fetch_source "$vixiecron_url" vixiecron-src.tar.gz
extract_source vixiecron-src.tar.gz vixiecron
cd vixiecron
# bitstring.h es un header de BSD que vixie cron da por hecho que existe
# (nació ahí); en Linux/glibc no está, así que le damos una implementación
# compatible mínima (es la API pública estándar de 4.4BSD, solo macros).
cat << 'BITSTRINGEOF' > bitstring.h
#ifndef _BITSTRING_H_
#define _BITSTRING_H_

typedef unsigned char bitstr_t;

#define _BITSTR_BITS 8

#define bitstr_size(nb) \
	((((nb) + _BITSTR_BITS - 1)) / _BITSTR_BITS)

#define bit_alloc(nb) \
	(bitstr_t *)calloc((size_t)bitstr_size(nb), sizeof(bitstr_t))

#define bit_decl(name, nb) \
	name[bitstr_size(nb)]

#define bit_clear(name, bit) \
	((name)[(bit) / _BITSTR_BITS] &= ~(1 << ((bit) % _BITSTR_BITS)))
...
#define bit_ffc(name, nbits, value) do { \
	int _b, _bit = -1; \
	for (_b = 0; _b < (nbits); _b++) { \
		if (!bit_test((name), _b)) { _bit = _b; break; } \
	} \
	*(value) = _bit; \
} while (0)
...
#define bit_nclear(name, start, stop) do { \
	int _i; \
	for (_i = (start); _i <= (stop); _i++) bit_clear((name), _i); \
} while (0)

#define bit_nset(name, start, stop) do { \
	int _i; \
	for (_i = (start); _i <= (stop); _i++) bit_set((name), _i); \
} while (0)

#endif /* !_BITSTRING_H_ */
BITSTRINGEOF
make CFLAGS="-static -O2 -I." LDFLAGS="-static" || { echo "** Falló la compilación de vixie cron."; exit 1; }
touch .built
cd ../..
fi

# isc-dhcp (dhclient): cliente DHCP clásico de Debian/RedHat, en vez de
# dhcpcd (más de Arch/BSD).
answer="n"
if [ -f files/isc-dhcp/.built ]; then
printf "** Se detectó una compilación previa de isc-dhcp. ¿Reutilizarla? (y/n): "
read answer
fi
if [ "$answer" != "y" ]; then
cd files
fetch_source "$isc_dhcp_url" isc-dhcp-src.tar.gz
extract_source isc-dhcp-src.tar.gz isc-dhcp
cd isc-dhcp
FORCE_UNSAFE_CONFIGURE=1 CFLAGS="-static -O2" LDFLAGS="-static" \
./configure --prefix=/usr \
|| { echo "** Falló el configure de isc-dhcp."; exit 1; }
make -j"$(nproc)" || { echo "** Falló la compilación de isc-dhcp."; exit 1; }
touch .built
cd ../..
fi
...
# OpenSSH: sshd -> sbin (servidor); ssh/scp/sftp/ssh-keygen/etc -> bin;
# sftp-server y sshd-session -> usr/libexec (helpers que sshd invoca
# internamente; desde OpenSSH 9.8, sshd delega cada conexión a
# sshd-session en vez de manejarla él mismo).
for b in sshd sshd-session ssh scp sftp sftp-server ssh-keygen ssh-agent ssh-add ssh-keyscan; do
src=$(find ../files/openssh -maxdepth 2 -type f -perm -u+x -name "$b" 2>/dev/null | head -1)
[ -z "$src" ] && continue
case "$b" in
sshd) cp "$src" sbin/ ;;
sftp-server|sshd-session) mkdir -p usr/libexec; cp "$src" usr/libexec/ ;;
*) cp "$src" bin/ ;;
esac
done
# Se compila dinámico (evita el lío de orden de libs estáticas contra
# libcrypto.a que tuvimos antes), así que TODOS sus binarios necesitan
# sus dependencias copiadas.
for b in sbin/sshd bin/ssh bin/scp bin/sftp usr/libexec/sftp-server usr/libexec/sshd-session bin/ssh-keygen bin/ssh-agent bin/ssh-add bin/ssh-keyscan; do
[ -f "$b" ] || continue
for lib in $(ldd "$b" 2>/dev/null | awk '{print $3}' | grep '^/'); do
cp -n "$lib" lib/ 2>/dev/null
done
done
mkdir -p etc/ssh
# /var/empty: directorio de chroot para el proceso sin privilegios de
# sshd (privilege separation). Tiene que ser de root y sin permiso de
# escritura para nadie más, o sshd directamente se niega a arrancar.
mkdir -p var/empty
chmod 711 var/empty
# isc-dhcp: dhclient -> sbin; dhclient-script (el script que realmente
# configura la IP/rutas/resolv.conf a partir de lo que dhclient
# negocia; dhclient por sí solo no toca la red) -> sbin también.
find ../files/isc-dhcp -maxdepth 3 -type f -perm -u+x -name dhclient -exec cp {} sbin/ \; 2>/dev/null
find ../files/isc-dhcp -maxdepth 4 -type f -name linux -path '*client/scripts*' -exec cp {} sbin/dhclient-script \; 2>/dev/null
chmod 755 sbin/dhclient-script 2>/dev/null
if [ -f sbin/dhclient ]; then
for lib in $(ldd sbin/dhclient 2>/dev/null | awk '{print $3}' | grep '^/'); do
cp -n "$lib" lib/ 2>/dev/null
done
fi
...
for b in syslogd hostname ifconfig ping ping6 traceroute logger; do
...
mkdir -p usr/sbin etc/nginx
src=$(find ../files/nginx -maxdepth 2 -type f -perm -u+x -name nginx 2>/dev/null | head -1)
[ -n "$src" ] && cp "$src" usr/sbin/nginx
if [ -f usr/sbin/nginx ]; then
for lib in $(ldd usr/sbin/nginx 2>/dev/null | awk '{print $3}' | grep '^/'); do
cp -n "$lib" lib/ 2>/dev/null
done
fi
cp ../files/nginx/conf/mime.types etc/nginx/mime.types 2>/dev/null

find ../files/vixiecron -maxdepth 2 -type f -perm -u+x \( -name 'cron' -o -name 'crond' \) -exec cp {} usr/sbin/cron \; 2>/dev/null
find ../files/vixiecron -maxdepth 2 -type f -perm -u+x -name 'crontab' -exec cp {} bin/ \; 2>/dev/null
...
int main(int argc, char **argv)
{
	...
	/* agetty invoca "login -- usuario"; telnetd invoca "login -h host"
	 * (sin usuario, solo para registrar el origen). Hay que saltar
	 * cualquier flag "-x" (y su valor, si "-h" lo trae) para no
	 * confundir un flag con el nombre de usuario. */
	while (idx < argc && argv[idx][0] == '-') {
		...
		if (strcmp(argv[idx], "-h") == 0)
			idx++; /* -h toma un argumento aparte: el hostname */
		idx++;
	}
...
printf "root:x:0:0:root:/root:/bin/sh\nservice:x:1:1:service:/var/www/html:/usr/sbin/nologin\nsshd:x:2:2:sshd privsep:/var/empty:/usr/sbin/nologin\n" > etc/passwd
lastchange_day=$(( $(date +%s) / 86400 ))
printf "root:%s:%s::::::\nservice:!:%s::::::\nsshd:!:%s::::::\n" "$root_hash" "$lastchange_day" "$lastchange_day" "$lastchange_day" > etc/shadow
...
<!DOCTYPE html><html lang="en"><head><title>$distro_name nginx default page: It works</title>
...
<p><b>$distro_name nginx</b> default page<br>ver. $version</p></body></html>
EOF
cat << EOF > etc/fstab
...
# Configuración mínima de dhclient (isc-dhcp) para $distro_name
timeout 60;
retry 10;
EOF
...
# nginx se demoniza (se va a segundo plano) por su cuenta salvo que se
# le diga lo contrario; init-functions ya lo backgroundea con "&", así
# que sin esto perderíamos el PID real, igual que nos pasó antes con
# dhcpcd y syslogd.
daemon off;
...
# nuestro propio nginx en /var/www/html/repo. Sirve para probar
...
cp var/spool/cron/crontabs/root var/cron/tabs/root
...
cat << 'EOF' > usr/sbin/service
#!/bin/sh
name="$1"

if [ -z "$name" ] || [ "$name" = "--status-all" ]; then
for initscript in /etc/init.d/*; do
[ -x "$initscript" ] || continue
n=$(basename "$initscript")
case "$n" in
rcS|rcK|init-functions|halt|reboot) continue ;;
esac
...
echo "service: no existe /etc/init.d/$name"
exit 1
fi
EOF
...
# -d: primer plano (init-functions ya lo backgroundea con "&"); sin
# esto dhclient se demoniza por su cuenta y el PID capturado sería el
# de un proceso que muere enseguida, no el de dhclient real (mismo
# problema que ya nos pasó antes con dhcpcd/syslogd).
exec /sbin/dhclient -d -sf /sbin/dhclient-script eth0 2>/dev/null
EOF

cat << 'EOF' > sbin/net-down
#!/bin/sh
/sbin/dhclient -r eth0 2>/dev/null
/sbin/ip link set eth0 down 2>/dev/null
EOF

cat << 'EOF' > sbin/sshd-start
#!/bin/sh
mkdir -p /etc/ssh
# ssh-keygen -A genera las host keys (RSA/ECDSA/Ed25519) que falten; si
# ya existen de un arranque anterior, no las toca. Así cada instalación
# tiene sus propias claves, en vez de compartir la misma que quedaría
# grabada en la ISO para todo el mundo.
/usr/bin/ssh-keygen -A >/dev/null 2>&1
# -D: primer plano (init-functions ya lo backgroundea con "&").
# -e: loguea a stderr, para que quede en /var/log/sshd.log.
exec /sbin/sshd -D -e -f /etc/ssh/sshd_config
EOF

initdata="
networking|network|30|/sbin/net-up||/sbin/net-down
cron|cron daemon|20|/usr/sbin/cron|-n||cron.log
syslogd|syslog|10|/sbin/syslogd|-n
nginx|servidor web (nginx)|40|/usr/sbin/nginx|||nginx.log
sshd|SSH server|50|/sbin/sshd-start|||sshd.log"
OIFS=$IFS
IFS='
'
...
;;
reload)
# No mandamos SIGHUP a ciegas: no todos los daemons la manejan igual
# (algunos clientes DHCP la usan para liberar la IP en vez de
# recargar). nginx y sshd sí soportan SIGHUP como recarga de verdad,
# pero un restart limpio es lo más seguro y predecible para cualquiera.
"$0" restart
;;
status)
if is_running; then
echo "$NAME está corriendo (pid $(cat $PIDFILE))."
return 0
else
echo "$NAME no está corriendo."
return 1
...
;;
*)
echo "$NAME init script usage: $0 {start|stop|restart|reload|status}"
return 1
esac
return $?
}
...
usr/sbin/install-to-disk sbin/net-up sbin/net-down sbin/sshd-start
chmod 644 etc/passwd etc/group etc/hostname etc/shells etc/hosts etc/protocols etc/services etc/syslog.conf etc/fstab etc/issue etc/motd etc/network/interfaces etc/profile etc/dhclient.conf etc/login.defs etc/wgetrc etc/pkg.conf etc/ssh/sshd_config etc/nginx/nginx.conf etc/nginx/mime.types
